...
public class StatePlaying{
    ...
    public void start(MazePanel panel) {
        ...
        seenCells = new Floorplan(GeneratingActivity.mazeConfig.getWidth()+1,GeneratingActivity.mazeConfig.getHeight()+1) ;
        ...
        Log.d("panel", panel.toString());
        if (panel != null) {
            Log.d("panel is", "not null");
            startDrawer();
        }
        else {
    ...
    protected void startDrawer() {
        firstPersonView = new FirstPersonView(Constants.VIEW_WIDTH,
                ...
                Constants.STEP_SIZE, seenCells, GeneratingActivity.mazeConfig.getRootnode()) ;
        mapView = new Map(seenCells, 15, GeneratingActivity.mazeConfig) ;
    ...
    private void setPositionDirectionViewingDirection() {
        ...
        int[] start = GeneratingActivity.mazeConfig.getStartingPosition() ;
    ...
    float getPercentageForDistanceToExit() {
        return GeneratingActivity.mazeConfig.getDistanceToExit(px, py) /
                ((float) GeneratingActivity.mazeConfig.getMazedists().getMaxDistance());
    }
    /**
    ...
    public Maze getMazeConfiguration() {
        return GeneratingActivity.mazeConfig ;
    }
    ...
    protected boolean checkMove(int dir) {
        ...
        return !GeneratingActivity.mazeConfig.hasWall(px, py, cd);
    }
    /**
    ...
    private boolean isOutside(int x, int y) {
        return !GeneratingActivity.mazeConfig.isValidPosition(x, y) ;
    }
    /**
    ...
    private boolean isFacingDeadEnd() {
        return (!isOutside(px,py) &&
                GeneratingActivity.mazeConfig.hasWall(px, py, getCurrentDirection()) &&
                GeneratingActivity.mazeConfig.hasWall(px, py, getCurrentDirection().oppositeDirection().rotateClockwise()) &&
                GeneratingActivity.mazeConfig.hasWall(px, py, getCurrentDirection().rotateClockwise()));
    }
